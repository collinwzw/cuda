...
__global__ void f_jocobiRelaxation( cudaPitchedPtr *A, cudaPitchedPtr *B, int n ){
    ...
    for (int z = 1; z < n -1; ++z) {
        ...
        for (int y = 1; y < n - 1; ++y) {
            ...
            for (int x = 1; x < n - 1; ++x) {
                A[x] =0.8 * ( B[x+1] + B[x - 1] + B[x + n] + B[x - n] + B[x + n * n] + B[x - n * n];
            }
        }
    }
...
int main(int argc, char* argv[]){
    ...
    cudaExtent extent = make_cudaExtent(n * sizeof(float),
                                        n, n);
    cudaPitchedPtr d_B;
    ...
    cpy.srcPtr = make_cudaPitchedPtr(h_B, n * sizeof(int), n, n);
    cpy.dstPtr = d_B;
    cpy.extent = extent;
