...
__global__ void f_jocobiRelaxation( float *A, float *B, int n, int zEnd ) {
    // kernel code might look something like this
    // but you may want to pad the matrices and index into them accordingly

    __shared__ float block[32][1][32];
    ...
    if (threadIdx.x < blockDim.x && threadIdx.y < blockDim.y && threadIdx.z< blockDim.z){
        block[threadIdx.x][threadIdx.y][threadIdx.z] = B[idx];
        //printf("when inde_block = %d, idx = %d\n", index_block, idx);
    }
    __syncthreads();

    // resolving the multuple
    if ((ix < n - 1) && (iy < n - 1) && (iz < zEnd) && (ix >= 1) && (iy >= 1) && (iz >= 1)) {
        if (threadIdx.x < blockDim.x - 1 && threadIdx.x >= 1 && threadIdx.y < blockDim.y - 1 && threadIdx.y >= 1 && threadIdx.z< blockDim.z-1 && threadIdx.z >= 1){
            A[idx] = block[threadIdx.x+1][threadIdx.y][threadIdx.z] + block[threadIdx.x-1][threadIdx.y][threadIdx.z] +
                    block[threadIdx.x][threadIdx.y+1][threadIdx.z] + block[threadIdx.x][threadIdx.y - 1][threadIdx.z] +
                    block[threadIdx.x][threadIdx.y][threadIdx.z+1] + block[threadIdx.x][threadIdx.y ][threadIdx.z- 1];
        }
        else{        A[idx] = (B[idx + 1] + B[idx - 1] +  B[idx + n] + B[idx - n] +
                               B[idx + n * n] + B[idx - n * n]);}

#ifdef DEBUG
        //printf("the addition at idx = %d in device: %.6f \n",idx, A[idx]);
        if (idx == 10101 ){
            printf("the B+1 at idx = %d in device: %lf \n",idx, B[idx+1]);
            printf("the B+1 at idx = %d in shared memory: %lf \n",idx, block[threadIdx.x+1][threadIdx.y][threadIdx.z]);
            printf("the addition at idx = %d in device: %.6f \n",idx, A[idx]);
...
//    }
    ...
    for( i = 1; i <=NSTREAMS; i++ ){
        ...
        //invoke kernel
        f_jocobiRelaxation<<<grid, block,0,stream[i]>>>( &d_A[offset], &d_B[offset], n,(blockZ-1) ) ;
        //record the timestamp before kernel invoke
    ...
    if(remainZ != 0){
        ...
        cudaEventRecord(start);
        debug = 0;
        f_jocobiRelaxation<<<grid, block,0,last>>>( &d_A[offset], &d_B[offset], n,remainZ - 1 ) ;
